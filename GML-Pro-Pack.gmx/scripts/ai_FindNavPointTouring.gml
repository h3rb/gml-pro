// Finds a benign place to go...
var o;
o=argument[0];
var ids,distances,i,found,rpick;
found=0;
...
var done,attempts,selected,limit;
attempts=0;
done=false;
selected=0;
if ( argument_count > 1 ) limit=argument[1];
else limit=room_width*2;
if ( found > 1 ) {
 while( !done and attempts < 10 ) {
  selected=number_range(0,found);
  if ( found == selected ) selected-=1;
  attempts+=1;
  if ( distances[selected] > limit ) done=true;
 }
}
